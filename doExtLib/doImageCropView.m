...
@property (nonatomic, assign) MidLineType type;
...
@end
@implementation doImageCropView
...
- (void) handleCornerPan:(UIPanGestureRecognizer *) panGesture {
    ...
    if (panView == _topLeftCorner) {
        approachAspectRatio = (MINY(panView)+self.cornerMargin)/(MINY(panView)+self.cornerMargin);
    }
    ...
    if (_cropAspectRatio > 0) {
        if (_cropAspectRatio >= approachAspectRatio) {
            ...
            spaceX = MAX(spaceX*_cropAspectRatio, currentMinSpace+_cropAreaCornerWidth*2-self.cornerMargin*2);
